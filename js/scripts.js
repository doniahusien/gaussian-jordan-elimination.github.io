...
function combineToTriangle() {
    for(a = 0; a < pubFunctionAmount; a++){
        ...
        for(b = 1 + a; b < pubFunctionAmount; b++){
            matrix.combineFunction(a, b, calcFactor(a, a, a, b)); //بيقسم الصف كله علي ليدنج   
            generateTableHTML();
        }
    }
...
function showX() {
    ...
    if(solutions === 1) {
        for(var i = 0; i <matrix.twoDimArr.length; ++i) {
            console.log("1")
        }
    }
    else if(solutions === 2) {
       
        console.log("DEPENDENT SYSTEM - INFINITE SOLUTIONS")
    }
    else {
       console.log( "INCONSISTENT SYSTEM - NO SOLUTIONS")
